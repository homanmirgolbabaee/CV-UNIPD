...
std::vector<cv::Point2f> BilliardAnalyzer::detectTableEdges() {
    ...
    if (horizontalLines.size() == 2 && verticalLines.size() == 2) {
        tablePoints.clear();  // Clear existing points
        tablePoints.push_back(getIntersection(horizontalLines[0], verticalLines[0]));
        tablePoints.push_back(getIntersection(horizontalLines[0], verticalLines[1]));
        tablePoints.push_back(getIntersection(horizontalLines[1], verticalLines[0]));
        tablePoints.push_back(getIntersection(horizontalLines[1], verticalLines[1]));
    } else {
...
void BilliardAnalyzer::initializeTracking(const cv::Mat& frame) {
    std::vector<cv::Point2f> ballCenters;
    std::vector<float> ballRadii;
    std::tie(ballCenters, ballRadii) = locateBalls(frame, tablePoints);
    ...
    classifyBalls(frame, ballCenters, ballRadii, whitePt, whiteRadius, blackPt, blackRadius,
                  stripedBalls, stripedRadii, solidBalls, solidRadii);

    currentBalls.clear();
    currentBalls.push_back({whitePt, whiteRadius, CUE});
    currentBalls.push_back({blackPt, blackRadius, EIGHT});

    for (size_t i = 0; i < stripedBalls.size(); ++i) {
        currentBalls.push_back({stripedBalls[i], stripedRadii[i], STRIPED});
    }

    for (size_t i = 0; i < solidBalls.size(); ++i) {
        currentBalls.push_back({solidBalls[i], solidRadii[i], SOLID});
    }

    ballTracker.initTrackers(frame, currentBalls);
}

std::vector<Ball> BilliardAnalyzer::trackBalls(const cv::Mat& frame) {
    std::vector<Ball> trackedBalls = ballTracker.updateTrackers(frame);
    
    std::vector<Ball> refinedBalls;
    for (const auto& ball : trackedBalls) {
        refinedBalls.push_back(refineBallPosition(frame, ball));
    }

    // If some balls are lost, try to re-detect them
    if (refinedBalls.size() < currentBalls.size()) {
        std::vector<cv::Point2f> ballCenters;
        std::vector<float> ballRadii;
        std::tie(ballCenters, ballRadii) = locateBalls(frame, tablePoints);
        ...
        for (size_t i = 0; i < ballCenters.size(); ++i) {
            bool isNewBall = true;
            for (const auto& trackedBall : refinedBalls) {
                if (cv::norm(ballCenters[i] - trackedBall.center) < trackedBall.radius) {
                    isNewBall = false;
                    break;
                }
            }
            if (isNewBall) {
                Ball newBall = {ballCenters[i], ballRadii[i], UNKNOWN};
                refinedBalls.push_back(refineBallPosition(frame, newBall));
            }
        }
    }

    currentBalls = refinedBalls;
    return refinedBalls;
}

Ball BilliardAnalyzer::refineBallPosition(const cv::Mat& frame, const Ball& ball) {
    cv::Mat ballROI = frame(cv::Rect(ball.center.x - ball.radius, ball.center.y - ball.radius,
                                     2 * ball.radius, 2 * ball.radius));
    cv::Mat grayROI;
    cv::cvtColor(ballROI, grayROI, cv::COLOR_BGR2GRAY);
    
    std::vector<cv::Vec3f> circles;
    cv::HoughCircles(grayROI, circles, cv::HOUGH_GRADIENT, 1, grayROI.rows / 2, 100, 30,
                     grayROI.rows / 4, grayROI.rows / 2);

    if (circles.size() > 0) {
        cv::Point2f refinedCenter(circles[0][0] + ball.center.x - ball.radius,
                                  circles[0][1] + ball.center.y - ball.radius);
        float refinedRadius = circles[0][2];
        return {refinedCenter, refinedRadius, ball.type};
    }
